define([
...
], function(KenticoConfig, Adapt, AdaptCollection, BuildModel, ConfigModel, CourseModel) {
  ...
  class Data extends AdaptCollection {
    ...
    /**
     * This method was customized by Kentico
     */
    async loadManifestFiles(languagePath) {
        // KENTICO CUSTOM START
        if (!KenticoConfig.serverUrl) {
            const errorMsg = `serverUrl is not configured. Please update 'kenticoConfig.js' file.`;
            alert(errorMsg)
            throw Error(errorMsg);
        }

        const envFolder = KenticoConfig.usePreviewEndpoint ? 'preview' : 'prod';
        const baseUrl = KenticoConfig.serverUrl + '/' + KenticoConfig.coursesPath + '/' + envFolder + '/' + KenticoConfig.courseId + '/' + KenticoConfig.language + '/';
      ...
      try {
        ...
        console.log('getting manifest', manifestPath);
      } catch (err) {
      ...
      const allFileData = await Promise.all(manifest.map(filePath => {
        return this.getJSON(`${baseUrl}/${filePath}`); // modified path that points to our servers
      }));
     // KENTICO CUSTOM END
